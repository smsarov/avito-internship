...
import { ItemsGetInQuerySchema, ItemSuggestInSchema, ItemUpdateInSchema } from "src/validation.ts";
import { treeifyError, ZodError } from "zod";
import { doesItemNeedRevision } from "./src/utils.ts";
import { gigachat } from "./src/gigachat.ts";
import { buildDescriptionPrompt, buildPricePrompt } from "./src/prompts.ts";
...
fastify.post("/items/suggestions/price", async (request, reply) => {
  ...
  try {
    const body = ItemSuggestInSchema.parse(request.body);
    const response = await gigachat.chat({
      messages: [{ role: "user", content: buildPricePrompt(body) }],
    });
    ...
    if (!Number.isFinite(price)) {
      reply.status(502).send({ success: false, error: "Model returned an unparseable price" });
      return;
    }
